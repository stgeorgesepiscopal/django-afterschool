    ...
    def form_valid(self, form):
        ...
        try:
            if Decimal(s.temperature) > Decimal(99.9):
                messages.error(self.request, f"<h3>Wait!</h3><span>Your temperature of ${s.temperature}&deg;F is above the safety threshold. Please contact Nurse Penny to help decide if you should be at work today.</span><h4>${str(s)}</h4>", extra_tags='safe')
            else:
...
class CheckoutView(FormView):
    ...
    def form_valid(self, form):
        ...
        try:
            ...
            logger.debug(e)
        self.success_url = reverse("carpool_location", kwargs={'location': form.cleaned_data['location']})
